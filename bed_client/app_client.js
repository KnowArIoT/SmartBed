...
socket.on('message', function(message) {
  ...
  if (message == "You are connected!") {
    ...
    if (message.dimValue) {
      ...
      httpGet('http://127.0.0.1:3000/dim/2/' + (message.dimValue * parseInt(2.5)));
    }
    else if (message.value) {
      console.log("calling toggleLight On");
      httpGet('http://127.0.0.1:3000/switch/2/on');
    }
    else if (!message.value) {
      console.log("calling toggleLight Off");
      httpGet('http://127.0.0.1:3000/switch/2/off');
    }
  }
    ...
    if (message.value) {
      ...
      httpGet('http://127.0.0.1:3000/switch/1/on');
    }
    else if (!message.value) {
      console.log("calling toggleHeat Off");
      httpGet('http://127.0.0.1:3000/switch/1/off');
    }
  }
  else if (message.type == "headAngle") {
    console.log("calling headAngle");
    httpGet('http://127.0.0.1:3000/angle/' + 'A/'+ (message.headAngleValue));
  }
  else if (message.type == "feetAngle") {
    console.log("calling feetAngle");
    httpGet('http://127.0.0.1:3000/angle/' + 'B/'+ (message.feetAngleValue));
  }
    ...
    httpGet('http://127.0.0.1:3000/scene/wakeup');
  }
  else if (message.type == "sleep") {
    console.log("calling sleep");
    httpGet('http://127.0.0.1:3000/scene/sleep');
  }
});
